...
*
*   \desc Runs all raven control functions. This is a thread run in parallel
*with
...
*
*	\brief This function controls the RAVEN for one loop cycle based on the
*desired control mode.
*
*	\desc This function first initializes the robot and then it computes
*Mpos and Velocities by calling
* 			stateEstimate() and then it calls fwdCableCoupling() and
*r2_fwd_kin() to calculate the
* 			forward cable coupling and inverse kinematics
*respectively.
* 			The following types of control can be selected from the
*control mode:
*  				-No control
...
*
* 	\pre  robot state is reflected in device0, DAC outputs are set in
*device0
...
int controlRaven(device *device0, param_pass *currParams) {
  ...
  switch (controlmode) {
    // this is handy for checking that gravity compensation works - also allows
    // for robot to be
    ...
    case motor_pd_control:
      ...
      break;
    // Runs joint velocity controller //ANDREW
    ...
    case homing_mode:
      static int hom = 0;
      if (hom == 0) {
        log_msg("RUNNING SNAKE RAVEN r2_control Entered homing mode");
        hom = 1;
      }
      initialized = false;
      ...
      break;
    // Runs applyTorque() to set torque command (tau_d) to a joint for debugging
    // purposes
...
/**
*	\fn int raven_cartesian_space_command(device *device0, param_pass
**currParams)
*
...
*  		2. call the invCableCoupling() to calculate the inverse cable
*coupling
*  		3. set all the joints to zero if pedal is not down otherwise it
*calls mpos_PD_control() to run the PD control law
*  		4. calls getGravityTorque() to calulate gravity torques on each
*joints.
...
/**
*	\fn raven_sinusoidal_joint_motion(device *device0, param_pass
**currParams)
*
*  	\brief  This function applies a sinusoidal trajectory to all joints
*
*	\desc This function:
*  			1. returns 0 if not in pedal down or init.init (do
*nothing)
*  			2. it sets trajectories on all the joints
*  			3. calls the invCableCoupling() to calculate inverse
*cable coupling
*  			4. calls the mpos_PD_control() to run the PD control law
*  			5. calls TorqueToDAC() to apply write torque value's on
*DAC
*
...
int raven_sinusoidal_joint_motion(device *device0, param_pass *currParams) {
  ...
  //    const float f_magnitude[8] = {0 DEG2RAD, 0 DEG2RAD, 0.0, 9999999, 0
  //    DEG2RAD, 25 DEG2RAD, 0 DEG2RAD, 0 DEG2RAD};
...
*
*  	\brief For debugging robot,  apply a set torque command (tau_d) to a
*joint.
*
*	\desc This function: only run in runlevel 1.2
*  			1. It checks the run level
*  			2. loops over all the joints and mechanisim to set the
*torque value
*  				MAX_DOF_PER_MECH is 8 and is defined in DS0.h
*  				NUM_MECH is the number of mechanisim of the
*robot
*
*  	\param device0 is robot_device struct defined in DS0.h
*  	\param currParams is param_pass struct defined in DS1.h
...
/**\
*	\fn raven_motor_position_control(device *device0, param_pass
**currParams)
*
*  	\brief This function runs PD control on motor position
*
*	\desc This function:
*    		1. checks to see if it's in pedal down mode, if not it sets
*all joints to zero torque and return 0
...
int raven_motor_position_control(device *device0, param_pass *currParams) {
  ...
  _joint = NULL;
  while (loop_over_joints(device0, _mech, _joint, i, j)) {
    if (_joint->type == SHOULDER_GOLD || _joint->type == ELBOW_GOLD) _joint->jpos_d = _joint->jpos;
...
/**
*	\fn raven_joint_velocity_control(device *device0, param_pass
**currParams)
*
* 	\brief This function runs pi_control on joint velocity
*
*	\desc This function:
*  			1. if pedal is down or RL_INIT and SL_AUTO_INIT, it
*Loops over all the joints and all the mechanisims
*  				to initialize velocity trajectory by calling
*start_trajectory() located in trajectory.cpp
*  			2. calls update_linear_sinusoid_velocity_trajectory() to
*get the desired joint velocities
*  			3. calls jvel_PI_control() to run PI control, which is
*in pid_control.cpp
*  			4. calls TorqueToDac() to apply torque on DAC
*  				It sets all the joint torques to zero if pedal is
*not down or not (RL_INIT and SL_AUTO_INIT)
*
* 	\param device0 is robot_device struct defined in DS0.h
* 	\param currParams is param_pass struct defined in DS1.h
*
...
int raven_joint_velocity_control(device *device0, param_pass *currParams) {
  ...
  static unsigned long int delay = 0;
    ...
    log_msg("*** Joint Velocity Control Mode ***");
...
// NEW RAVEN_JOINT_VELOCITY_CONTROL!!!!!!!!!!!!!!!!11
int raven_joint_velocity_control(device *device0, param_pass *currParams) {
  static int controlStart = 0;
  static unsigned long int delay = 0;
  const float f_period[8] = {6, 7, 10, 9999999, 10, 5, 10, 6};
  //    const float f_magnitude[8] = {0 DEG2RAD, 0 DEG2RAD, 0.0, 9999999, 0
  //    DEG2RAD, 25 DEG2RAD, 0 DEG2RAD, 0 DEG2RAD};
  const float f_magnitude[8] = {10 DEG2RAD, 10 DEG2RAD, 0.03,       9999999,
                                30 DEG2RAD, 30 DEG2RAD, 30 DEG2RAD, 30 DEG2RAD};
  ...
  for (int i = 0; i < NUM_MECH; i++) {
    for (int j = 0; j < MAX_DOF_PER_MECH; j++) {
	  ...
	  //GET UPDATE VALUE FOR EACH JOINT IN THIS LOOP FROM ROS
      ...
      // Get trajectory update from deltas_array
      update_joint_position_trajectory(_joint);
